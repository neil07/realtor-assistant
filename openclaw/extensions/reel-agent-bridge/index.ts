...
type EventContext = {
  api: Parameters<
    Exclude<Parameters<typeof definePluginEntry>[0]["register"], undefined>
  >[0];
  config: BridgeConfig;
  statePath: string;
};
...
function defaultMediaLocalRoots(repoRoot: string): string[] {
  return [
    repoRoot,
    path.join(repoRoot, "skills", "listing-video", "output"),
    "/tmp",
  ];
}
...
function normalizePluginConfig(
  ...
  const callbackSecret =
    typeof raw?.callbackSecret === "string" ? raw.callbackSecret.trim() : "";
  if (!callbackSecret) {
    throw new Error(
      "reel-agent-bridge requires plugins.entries.reel-agent-bridge.config.callbackSecret",
    );
  }
  ...
  return {
    ...
    repoRoot,
    workspaceStatePath:
      typeof raw?.workspaceStatePath === "string" &&
      raw.workspaceStatePath.trim()
...
async function loadRepoEnv(repoEnvPath: string): Promise<BackendConfig> {
  ...
  const reelAgentUrl = (
    env.REEL_AGENT_URL ||
    process.env.REEL_AGENT_URL ||
    ""
  ).trim();
  const reelAgentToken = (
    env.REEL_AGENT_TOKEN ||
    process.env.REEL_AGENT_TOKEN ||
    ""
  ).trim();
  if (!reelAgentUrl || !reelAgentToken) {
    throw new Error(
      "Missing REEL_AGENT_URL or REEL_AGENT_TOKEN for reel-agent-bridge router",
    );
  }
  ...
  return {
    ...
    callbackBaseUrl:
      (
        env.OPENCLAW_CALLBACK_BASE_URL ||
        process.env.OPENCLAW_CALLBACK_BASE_URL ||
        ""
      ).trim() || undefined,
    dailyTriggerSecret:
      (
        env.DAILY_TRIGGER_SECRET ||
        process.env.DAILY_TRIGGER_SECRET ||
        ""
      ).trim() || undefined,
  };
}
...
async function backendFetchJson(
  ...
  const response = await fetch(
    `${backend.reelAgentUrl.replace(/\/$/, "")}${pathName}`,
    {
      ...init,
      headers,
    },
  ...
  if (!response.ok) {
    throw new Error(
      `Backend ${response.status} ${pathName}: ${typeof body?.detail === "string" ? body.detail : text}`,
    );
  }
  return body;
}
...
async function loadProfileStyle(
  backend: BackendConfig,
  agentPhone: string,
): Promise<ListingStyle | undefined> {
  try {
    const body = await backendFetchJson(
      backend,
      `/api/profile/${encodeURIComponent(agentPhone)}`,
      {
        method: "GET",
      },
    );
...
function pickTelegramTarget(event: {
  ...
  if (chatId) {
    return event.isGroup && Number.isFinite(threadId)
...
function resolveBinding(
  state: BridgeState,
  payload: BridgePayloadBase,
): TargetBinding | undefined {
  if (
    payload.openclaw_msg_id &&
    state.routesByMessageId[payload.openclaw_msg_id]
  ) {
...
function renderDeliveredText(payload: DeliveredPayload): string {
  const stats = [
    typeof payload.scene_count === "number"
      ...
      : "",
    typeof payload.word_count === "number"
...
function renderDeliveredPublishText(lastDelivery?: LastDelivery): string {
  ...
  lines.push(
    "",
    "If you want another cut, reply with:",
    "- adjust <what to change>",
    "- redo",
  );
  return lines.join("\n");
...
function isHelpLike(text: string): boolean {
  if (
    ["what can you do", "what do you do"].some((token) => text.includes(token))
  ) {
    return true;
  }
  return ["help", "hi", "hello", "start"].some((token) =>
    new RegExp(`(^|\\s)${token}(\\s|$)`, "i").test(text),
  );
}

function isTrustFirstQuestion(text: string): boolean {
  return ["is this an app", "secure", "how much", "price", "first step"].some(
    (token) => text.includes(token),
  );
}

function isDailyInsightRequest(text: string): boolean {
  return ["daily insight", "market insight", "market update", "insight"].some(
    (token) => text.includes(token),
  );
}
...
function isPropertyContentText(text: string): boolean {
  ...
  const hasAddressLike =
    /\d+\s+.+(st|street|ave|avenue|rd|road|dr|drive|blvd|lane|ln)\b/i.test(
      text,
    );
...
function buildCallbackUrl(backend: BackendConfig): string | undefined {
  return backend.callbackBaseUrl
    ? `${backend.callbackBaseUrl.replace(/\/$/, "")}/events`
    : undefined;
}

function renderStyleSelectionReply(photoCountHint?: number): string {
  const photoLine =
    typeof photoCountHint === "number"
      ? `Got ${photoCountHint} photos. `
      : "Got your photos. ";
  return [
...
async function restartListingVideoJob(
  ...
  return startListingVideoJob(
    backend,
    senderId,
    lastInput.firstPhotoPath,
    lastInput.style,
  );
}

function isStopPush(text: string): boolean {
  return ["stop push", "pause push", "disable daily"].some((token) =>
    text.includes(token),
  );
}

function isResumePush(text: string): boolean {
  return ["resume push", "start push", "enable daily"].some((token) =>
    text.includes(token),
  );
}
...
function renderDailyInsightText(payload: DailyInsightPayload): string {
  ...
  const hashtags =
    Array.isArray(insight.hashtags) && insight.hashtags.length > 0
      ? insight.hashtags.join(" ")
      : "";
...
function ensureRenderableDailyInsight(payload: DailyInsightPayload): void {
  ...
  const imageUrls = payload.image_urls
    ? Object.values(payload.image_urls).filter(
...
async function sendTelegramMessage(
  ...
  await send(binding.target, text, {
    ...
    ...(binding.replyToMessageId
      ? { replyToMessageId: binding.replyToMessageId }
      : {}),
    ...(mediaUrl ? { mediaUrl } : {}),
    ...(config.mediaLocalRoots.length > 0
  ...
  switch (type) {
    case "progress":
      return {
        ...
        agent_phone:
          typeof payload.agent_phone === "string"
            ? payload.agent_phone.trim()
            : undefined,
        openclaw_msg_id:
          typeof payload.openclaw_msg_id === "string"
            ? payload.openclaw_msg_id.trim()
            : undefined,
        step:
          typeof payload.step === "string" ? payload.step.trim() : undefined,
        message:
      ...
      return {
        ...
        video_url:
          typeof payload.video_url === "string"
            ...
            : undefined,
        video_path:
          typeof payload.video_path === "string"
            ? payload.video_path.trim()
            : undefined,
        caption:
      ...
      return {
        ...
        retry_count:
          typeof payload.retry_count === "number"
            ...
            : undefined,
        override_url:
          typeof payload.override_url === "string"
            ? payload.override_url.trim()
            : undefined,
      };
    case "daily_insight":
      return {
        ...
        openclaw_msg_id:
          typeof payload.openclaw_msg_id === "string"
            ...
            : undefined,
        insight:
          typeof payload.insight === "object" && payload.insight
            ? (payload.insight as DailyInsightPayload["insight"])
            : undefined,
        image_urls:
      ...
      return {
        ...
        agent_name:
          typeof payload.agent_name === "string"
            ...
            : undefined,
        form_url:
          typeof payload.form_url === "string" ? payload.form_url : undefined,
        message:
...
async function handleBridgePayload(
  ctx: EventContext,
  state: BridgeState,
  ...
  switch (payload.type) {
    ...
    case "daily_insight": {
      ...
      const imageRef = payload.image_urls
        ? Object.values(payload.image_urls).find(
      ...
      await sendTelegramMessage(
        ...
        ctx.config,
        binding,
        renderDailyInsightText(payload),
        imageRef,
      );
      if (payload.agent_phone) {
        upsertAgentState(state, payload.agent_phone, binding, {
    ...
    case "onboarding_form": {
      await sendTelegramMessage(
        ...
        ctx.config,
        binding,
        renderOnboardingFormText(payload),
      );
      break;
    }
    case "form_completed": {
      await sendTelegramMessage(
        ...
        ctx.config,
        binding,
        renderFormCompletedText(payload),
      );
      break;
    }
  }
  ...
  if (payload.type === "daily_insight" && payload.agent_phone) {
    ...
    if (
      sessionContext?.lastInsightPublishedAt &&
      !sessionContext.videoHandoffNudgedAt
    ) {
      await sendTelegramMessage(
        ctx.api,
        ctx.config,
...
async function handleRouterMessage(
  ...
  handled: boolean;
  text?: string;
  reason: RouterDebugEntry["reason"];
  normalizedText?: string;
}> {
  ...
  if (mediaPath && mediaType?.startsWith("image/")) {
    ...
    if (profileStyle) {
      const job = await startListingVideoJob(
        backend,
        senderId,
        mediaPath,
        profileStyle,
      ...
      upsertAgentState(state, senderId, binding, {
        lastJobId:
          typeof job?.job_id === "string" ? job.job_id : agentState?.lastJobId,
  ...
  if (
    pendingListing?.awaiting === "confirmation" &&
    pendingListing.style &&
    isConfirm(text)
  ) {
    const job = await startListingVideoJob(
      backend,
      senderId,
      pendingListing.firstPhotoPath,
      pendingListing.style,
    );
    upsertAgentState(state, senderId, binding, {
      lastJobId:
  ...
  if (isTrustFirstQuestion(text)) {
    return {
      handled: true,
      text: renderTrustFirstReply(),
      reason: "handled_trust_first",
      normalizedText: text,
    };
  }

  if (isStopPush(text) || isResumePush(text)) {
    const action = isStopPush(text)
      ? "disable_daily_push"
      : "enable_daily_push";
  ...
  if (isDailyInsightRequest(text)) {
    await backendFetchJson(
      backend,
      `/api/daily-trigger?secret=${encodeURIComponent(
        backend.dailyTriggerSecret ?? "",
      )}`,
      {
        method: "POST",
      },
  ...
  if (sessionContext.lastPostRenderKind === "daily_insight") {
    ...
    if (isInsightRefineShorter(text) || isInsightRefineProfessional(text)) {
      const feedbackText = isInsightRefineShorter(text)
        ? "shorter"
        : "more professional";
  ...
  if (
    sessionContext.lastPostRenderKind === "delivered" &&
    agentState?.lastJobId
  ) {
    if (isPublish(text)) {
      upsertAgentState(state, senderId, binding, {
        sessionContext: {
    ...
    if (isRedo(text)) {
      const job = await restartListingVideoJob(
        backend,
        senderId,
        agentState?.lastListingVideoInput,
      );
      if (!job) {
        return {
          handled: true,
...
export default definePluginEntry({
  ...
  register(api) {
    ...
    api.logger.info?.(
      `${PLUGIN_ID}: register account=${config.telegramAccountId} statePath=${statePath}`,
    );

    api.on("gateway_start", () => {
      api.logger.info?.(
        `${PLUGIN_ID}: gateway_start account=${config.telegramAccountId}`,
      );
    });
    ...
    api.on("before_dispatch", async (event, hookCtx) => {
      if (
        ...
        hookCtx.accountId !== config.telegramAccountId ||
        event.isGroup
      ) {
        return;
      }
      ...
      if (!senderId || !conversationId) {
        ...
        state.lastRouterDebug = {
          ...
          normalizedText: (event.content ?? event.body ?? "")
            .trim()
            .toLowerCase(),
          handled: false,
        ...
        await persistState(statePath, state, config.workspaceStatePath);
        api.logger.info?.(
          `${PLUGIN_ID}: before_dispatch skipped missing sender/conversation`,
        );
        return;
      }
    ...
    api.registerHttpRoute({
      ...
      async handler(req, res) {
        ...
        try {
          ...
          const status = message.includes("No Telegram target binding")
            ? 404
